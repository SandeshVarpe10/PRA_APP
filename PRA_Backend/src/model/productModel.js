...
exports.updateProductSave = (
  ...
  return new Promise((resolve, reject) => {
    db.query("UPDATE products SET product_name = ?, product_image = ?, subcategory_id = ?, brand = ?, description = ?, stock_unit = ?, stock = ?, price = ?, discount = ?, organic = ? WHERE product_id = ? ",
         [name, image, subcategoryId, brand, description, stockUnit, stock, price, discount, organic, productId], (err, result) => {
      if (err) reject(err);
      else resolve(result);
    });
  });
...
exports.deleteSubCatByID = (id) => {
  return new Promise((resolve, reject) => {
    db.query("DELETE FROM subcategory WHERE subcategory_id = ?", [id], (err, result) => {
      if (err)
      {
         reject(err);
      }
