...
class Service {
  ...
  updSubCategoryById(Cid, Sid, formData) {
    return axios.post(
      `http://localhost:3000/updatesubcategory/${Cid}/${Sid}`,
      formData,
      {
  ...
  updateProduct(product_id, data) {
    return axios.post(
      `http://localhost:3000/productupdatesave/${product_id}`,
      data
    );
  }
  ...
  searchProductsBySubcategory(query, subcategoryId) {
    return axios.get(
      `http://localhost:3000/searchProductByCat/${query}/${subcategoryId}`
    );
  }
}
